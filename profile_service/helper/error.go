...
func MappingValidationErros(exception validator.ValidationErrors) map[string]string {
	...
	for _, fieldError := range exception {
		...
		case "startsnotwith":
			value = fmt.Sprintf("%s must begin with the country area code", fieldName)
		case "nefield":
			value = fmt.Sprintf("%s cannot be the same as %s", fieldName, fieldError.Param())
		case "excludes":
			value = fmt.Sprintf("%s must not contain spaces", fieldName)
		}
